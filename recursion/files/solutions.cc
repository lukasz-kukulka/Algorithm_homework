...
void Window::lukaszShape(Matrix& matrix, Point current, int size) {
    ...
    fillLine(matrix, right - Point{1, 0}, left);
    fillLine(matrix, left, top);  
    fillLine(matrix, top, right);

    print2D(matrix, os_);
    std::this_thread::sleep_for(std::chrono::seconds(2));
    lukaszShape(matrix, top - Point{0, 3}, size - 10);
}

void Window::drawLukaszShape() {
    Matrix matrix(size_ + 1, Row(size_ + 1, ' '));
    lukaszShape(matrix, {(int)(size_ / 2), (int)size_}, size_);
    print2D(matrix, os_);
}
